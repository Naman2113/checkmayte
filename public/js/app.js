...
//If win or draw
socket.on('gameOver', (turn, win, nftToken) => {
    config.draggable = false;
    if (win) {
        if (board.orientation().includes(turn)) {
            ...
            statusEl.innerHTML = "Congratulations, you won!! NFT for this game created at <b class=\"heart\">" + nftToken + "</b>"
        }
    }
    else {
...
joinButtonEl.addEventListener('click', (e) => {
    ...
    var key = window.solana.publicKey.toString()
    ...
    if (!user || !room) {
        ...
        socket.emit('joinRoom', { user, room, key }, (error) => {
...
window.solana.on("connect", () => {
    ...
    document.getElementById('userId').value = window.solana.publicKey.toString();
    }
)
